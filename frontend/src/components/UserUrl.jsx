...
const UserUrl = () => {
  ...
  // If data not loaded or empty, handle loading or empty state
  if (isLoading) {
    return (
  ...
  // Pagination logic
  const totalItems = urls.urls.length;
  const totalPages = Math.ceil(totalItems / itemsPerPage);

  const paginatedUrls = urls.urls
    .slice()
    .reverse()  // Reverse first so newest on top
  ...
  return (
    <div className="bg-white rounded-lg mt-5 overflow-hidden">
      ...
      <div className="overflow-x-auto max-h-[520px]"> {/* vertical scroll if content too tall */}
